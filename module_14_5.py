...
async def get_buying_list(message):
    ...
    for product in products:
        ...
        with open(f'files/product{product[0]}.png', 'rb') as img:
...
async def set_age(message, state):
    ...
    add_user(all_state['username'], all_state['email'], all_state['age'])
    await message.answer(f'Успешная регистрация {all_state["username"]}')
    await state.finish()
